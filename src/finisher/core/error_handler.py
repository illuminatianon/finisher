...
class NetworkError(FinisherError):
    ...
    def __init__(self, message: str, user_message: Optional[str] = None, **kwargs):
        super().__init__(
            ...
            user_message=user_message or "Network connection error. Please check your connection and Auto1111 server.",
            **kwargs
        )
...
class ImageError(FinisherError):
    ...
    def __init__(self, message: str, user_message: Optional[str] = None, **kwargs):
        super().__init__(
            ...
            user_message=user_message or "Image processing error. Please check the image format and try again.",
            **kwargs
        )
...
class FileError(FinisherError):
    ...
    def __init__(self, message: str, file_path: Optional[str] = None,
        ...
        super().__init__(
            ...
            user_message=user_message or "File error. Please check the file path and permissions.",
            details=details,
            **kwargs
        )
...
class ProcessingError(FinisherError):
    ...
    def __init__(self, message: str, user_message: Optional[str] = None, **kwargs):
        super().__init__(
            ...
            user_message=user_message or "Processing error. The upscaling operation failed.",
            **kwargs
        )
...
class ErrorHandler:
    ...
    def handle_exception(self, exc: Exception, context: str = "", 
                        show_dialog: bool = True) -> FinisherError:
    ...
    def handle_network_error(self, exc: requests.RequestException, 
                           context: str = "") -> NetworkError:
        ...
        if isinstance(exc, requests.Timeout):
            error = NetworkError(
                ...
                user_message="Request timed out. The Auto1111 server may be busy or unreachable."
            )
        elif isinstance(exc, requests.ConnectionError):
            error = NetworkError(
                f"Connection error: {exc}",
                user_message="Cannot connect to Auto1111 server. Please check if it's running and accessible."
            )
            ...
            error = APIError(
                ...
                user_message=f"Auto1111 server returned error {status_code}. Please check server logs."
            )
        else:
    ...
    def handle_image_error(self, exc: Exception, file_path: Optional[str] = None,
                          context: str = "") -> ImageError:
        ...
        if "cannot identify image file" in str(exc).lower():
            user_message = "Invalid image file. Please select a valid image format (PNG, JPEG, etc.)."
        elif "truncated" in str(exc).lower():
            user_message = "Corrupted image file. The image appears to be incomplete or damaged."
        elif "size" in str(exc).lower():
            user_message = "Image size error. The image may be too large or have invalid dimensions."
        else:
    ...
    def handle_file_error(self, exc: Exception, file_path: Optional[str] = None,
                         context: str = "") -> FileError:
        ...
        if isinstance(exc, FileNotFoundError):
            ...
            user_message = f"Permission denied accessing file: {file_path or 'Unknown file'}"
