...
class BatchInputHandler:
    ...
    def handle_multiple_files(self, file_paths: List[str], 
                             batch_name: Optional[str] = None) -> Tuple[List[str], List[str], str]:
        ...
        if not valid_files:
            error_msg = f"No valid image files found in batch of {len(file_paths)} files"
    ...
    def handle_directory_drop(self, directory_path: str, 
                             recursive: bool = True,
                             batch_name: Optional[str] = None) -> Tuple[List[str], List[str], str]:
    ...
    def validate_batch_input(self, file_paths: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for i, file_path in enumerate(file_paths):
            # Report progress
            if self.on_progress:
                self.on_progress(f"Validating {os.path.basename(file_path)}", i + 1, total_files)
            ...
            try:
                ...
                if file_size > self.max_file_size:
                    ...
                    invalid_files.append(f"{file_path}: File too large ({size_mb:.1f}MB > {max_mb:.1f}MB)")
                    continue
    ...
    def get_batch_summary(self, valid_files: List[str], invalid_files: List[str]) -> Dict[str, Any]:
