...
class UpscalingPipeline:
    ...
    def start_upscaling(self, image_path: str, config: ProcessingConfig) -> bool:
        ...
        self.processing_thread = threading.Thread(
            target=self._process_image, args=(image_path, config), daemon=True
        )
    ...
    def start_upscaling_from_data(
        self, image_data: bytes, config: ProcessingConfig
    ) -> bool:
        ...
        self.processing_thread = threading.Thread(
            target=self._process_image_data, args=(image_data, config), daemon=True
        )
        self.processing_thread.start()
    ...
    def _process_image(self, image_path: str, config: ProcessingConfig) -> None:
        ...
        try:
            ...
            base64_image, prompt, negative_prompt, width, height = (
                self.processor.prepare_image_for_processing(image_path)
            )

            # Execute two-pass pipeline
            self._execute_pipeline(
                base64_image, prompt, negative_prompt, config, width, height
            )
    ...
    def _process_image_data(self, image_data: bytes, config: ProcessingConfig) -> None:
        ...
        try:
            ...
            base64_image, prompt, negative_prompt, width, height = (
                self.processor.prepare_image_data_for_processing(image_data)
            )
    ...
    def _execute_pipeline(
        ...
        try:
            ...
            try:
                ...
                first_pass_path = os.path.join(
                    temp_dir, f"finisher_first_pass_{self.current_job_id}.png"
                )
            ...
            if "image" in second_pass_result:
                ...
                try:
                    ...
                    output_path = os.path.join(
                        temp_dir, f"finisher_result_{self.current_job_id}.png"
                    )
