...
class StatusMonitor:
    ...
    def __init__(self, client: Auto1111Client):
        ...
        # Callbacks
        self.on_status_changed: Optional[
            Callable[[JobStatus, float, Optional[float], Optional[str]], None]
        ] = None
    ...
    def _check_status(self) -> None:
        ...
        try:
            ...
            if (
                ...
                or abs(progress_info.progress - self.current_progress) > 0.01
            ):
            ...
            if (
                self.current_status in [JobStatus.PROCESSING, JobStatus.FINALIZING]
                and new_status == JobStatus.IDLE
                and self.current_job_timestamp
            ):
    ...
    def _determine_job_status(self, progress_info: ProgressInfo) -> JobStatus:
        ...
        is_our_job = self._is_our_job(
            progress_info.state.job_timestamp if progress_info.state else None
        )
    ...
    def _handle_error(self, error_message: str) -> None:
        ...
        logger.error(
            f"Status monitor error ({self.consecutive_errors}): {error_message}"
        )
