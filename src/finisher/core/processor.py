...
from .utils import encode_image_to_base64, decode_base64_to_image, validate_image_format
...
class ImageProcessor:
    ...
    def validate_image_dimensions(self, image: Image.Image, 
                                max_width: int = 4096, 
                                max_height: int = 4096) -> bool:
        ...
        if width < 64 or height < 64:
            logger.warning(f"Image dimensions {width}x{height} too small (minimum 64x64)")
            return False
        
        return True
    
    def resize_image_if_needed(self, image: Image.Image, 
                             max_dimension: int = 2048) -> Image.Image:
