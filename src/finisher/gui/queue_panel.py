...
class QueueJobItem(QListWidgetItem):
    ...
    def update_display(self) -> None:
        ...
        self.setText(
            f"{state_icon} {display_name}{progress_text}{batch_text}{priority_text}"
        )
        ...
        if is_dark_mode:
            ...
            if self.job.state == JobState.RUNNING:
                ...
                if self.job.priority > 0:
                    self.setBackground(
                        QColor(255, 248, 220)
                    )  # Light yellow for high priority
...
class QueuePanel(QWidget):
    ...
    def _on_selection_changed(self) -> None:
        ...
        can_reorder = (
            current_item
            and isinstance(current_item, QueueJobItem)
            and current_item.job.state == JobState.QUEUED
        )
    ...
    def _show_context_menu(self, position) -> None:
        ...
        if job.state == JobState.QUEUED:
            ...
            high_priority_action.triggered.connect(
                lambda: self._set_job_priority(job.id, 10)
            )
            menu.addAction(high_priority_action)

            normal_priority_action = QAction("Set Normal Priority", self)
            normal_priority_action.triggered.connect(
                lambda: self._set_job_priority(job.id, 0)
            )
            menu.addAction(normal_priority_action)

            low_priority_action = QAction("Set Low Priority", self)
            low_priority_action.triggered.connect(
                lambda: self._set_job_priority(job.id, -10)
            )
    ...
    def _cancel_job(self, job_id: str) -> None:
        ...
        if self.queue_manager:
            ...
            if not success:
                QMessageBox.warning(
                    self, "Cancel Failed", f"Failed to cancel job {job_id}"
                )
    ...
    def _retry_job(self, job_id: str) -> None:
        ...
        QMessageBox.information(
            self, "Retry Job", "Job retry functionality not yet implemented"
        )
    ...
    def _set_job_priority(self, job_id: str, priority: int) -> None:
        ...
        if self.queue_manager:
            ...
            if job and job.state == JobState.QUEUED:
                ...
                # Re-sort the queue
                self.queue_manager.job_queue.sort(
                    key=lambda j: j.priority, reverse=True
                )
    ...
    def _clear_completed(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            QMessageBox.information(
                self, "Jobs Cleared", f"Cleared {count} completed jobs"
            )
