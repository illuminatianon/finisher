...
class Auto1111Client:
    ...
    def get_upscalers(self) -> List[Dict[str, Any]]:
        ...
        """
        response = self.session.get(f"{self.base_url}/sdapi/v1/upscalers", timeout=30)
    ...
    def get_models(self) -> List[Dict[str, Any]]:
        ...
        Raises:
            requests.RequestException: If API call fails
        """
        response = self.session.get(f"{self.base_url}/sdapi/v1/sd-models", timeout=30)
        response.raise_for_status()
        return response.json()
    ...
    def get_samplers(self) -> List[Dict[str, Any]]:
        ...
        Raises:
            requests.RequestException: If API call fails
        """
        response = self.session.get(f"{self.base_url}/sdapi/v1/samplers", timeout=30)
        response.raise_for_status()
        return response.json()
    ...
    def get_schedulers(self) -> List[Dict[str, Any]]:
        ...
        Raises:
            requests.RequestException: If API call fails
        """
        response = self.session.get(f"{self.base_url}/sdapi/v1/schedulers", timeout=30)
        response.raise_for_status()
        return response.json()
    ...
    def img2img(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "script_args" in payload:
            types = [type(arg).__name__ for arg in payload["script_args"]]
            logger.info(f"script_args: {payload['script_args']} (types: {types})")
    ...
    def health_check(self) -> bool:
        ...
        """
        try:
            response = self.session.get(f"{self.base_url}/sdapi/v1/memory", timeout=10)
